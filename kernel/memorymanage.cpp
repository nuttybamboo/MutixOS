...
void MemoryManage::MemoryManageInit()
{
    ...
    memory_map = (char*)(g_memory->get_address(MM_BASE + MM_LIMIT - sizeof(char)*PAGING_PAGES));
    //printf("getting address of memory_map= MM_BASE=%u + MM_LIMIT=%u - sizeof(char)*PAGING_PAGES=%u  = %u is %u\n",
    //       MM_BASE ,MM_LIMIT, PAGING_PAGES, MM_BASE + MM_LIMIT - sizeof(char)*PAGING_PAGES, memory_map);
    memset(memory_map, PAGING_PAGES, 0);
    ...
    for(size_t i = 0; i < KERNEL_MEM_END/PAGE_SIZE; ++i)
    {
        write_page_table(i * PAGE_SIZE, i * PAGE_SIZE);
    }
...
unsigned long MemoryManage::ldt_copy_mem(int index, int current_index)
{
    ...
    //printf("before copy_page_tables\n");
...
bool MemoryManage::copy_page_tables(unsigned long source_base, unsigned long dect_base, unsigned long limit)
{
    ...
    for ( int j = 0; size > j; j++ )
    {
        ...
        if (!(1 & (currentMM -> page_dir[source_dir + j]) ))
            continue;
        source_table = (unsigned long *) g_memory->get_address(0xfffff000 & (currentMM -> page_dir[source_dir + j]) );

        //the page_table should not exist...we just create it...
        if (!(tmp=get_free_page()))
            return false;
        currentMM -> page_dir[dect_dir + j] = tmp | 7;
        dect_table = (unsigned long *) g_memory->get_address(tmp);
...
{
    //printf("getting free page, memory_map base is %u\n", currentMM -> memory_map);
...
void MemoryManage::put_page(unsigned long liner_address)
{
    ...
    //printf("in puting page , liner address=%u, get phsical address=%u\n", liner_address, physical_address);
...
bool MemoryManage::write_page_table(unsigned long address, unsigned long page)
{
    ...
    if (( currentMM -> page_dir[table_item_offset] ) & 1)
        page_table = (unsigned long *) g_memory->get_address(0xfffffff8 & (currentMM -> page_dir[table_item_offset]) );
    else
    {
        ...
        page_table = (unsigned long *) g_memory->get_address(tmp);
    }
    ...
    //printf("put page , page table base is %u, index is %d, value is %u\n",
    //       page_table, (address >> 12) &0x3ff, page_table + ((address>>12) & 0x3ff));
    page_table[(address>>12) & 0x3ff] = page | 7;
    //printf("we a page with linner addr %u in the physical addr %u\n",
    //   address, page);
...
//*
    address &= 0xfffff000;
    tmp = address - ProcessManage::getCurrentTaskAddr() -> getStart_code();
    if (tmp >= ProcessManage::getCurrentTaskAddr() -> getEnd_data())
    {
